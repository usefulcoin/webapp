...
export function convertToDecimals(value: string | number, decimals: any): string {
  const i = new BigNumber(value);
  return i.toFormat(parseInt(decimals, 10));
}

export function bigNumberStringToInt(value: string): number {
  const i = value.replace('.', "");
